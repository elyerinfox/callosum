...
const DEFAULT_CALLOSUM_METAL_COMPUTE_PER_BUFFER: usize = 50;
const DEFAULT_CALLOSUM_METAL_COMMAND_POOL_SIZE: usize = 5;
...
impl Commands {
    pub fn new(command_queue: CommandQueue) -> Result<Self, MetalKernelError> {
        let compute_per_buffer = match std::env::var("CALLOSUM_METAL_COMPUTE_PER_BUFFER") {
            Ok(val) => val
                .parse()
                .unwrap_or(DEFAULT_CALLOSUM_METAL_COMPUTE_PER_BUFFER),
            _ => DEFAULT_CALLOSUM_METAL_COMPUTE_PER_BUFFER,
        };

        let pool_size = match std::env::var("CALLOSUM_METAL_COMMAND_POOL_SIZE") {
            Ok(val) => val
                .parse()
                .unwrap_or(DEFAULT_CALLOSUM_METAL_COMMAND_POOL_SIZE),
            _ => DEFAULT_CALLOSUM_METAL_COMMAND_POOL_SIZE,
        };
