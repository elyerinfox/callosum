...
impl Embedding {
    pub fn new(d1: usize, d2: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let inner = callosum_nn::Embedding::new(embeddings, d2);
...
impl Module for Linear {
    fn forward(&self, x: &Tensor) -> callosum::Result<Tensor> {
...
pub fn layer_norm(size: usize, eps: f64, vb: VarBuilder) -> Result<callosum_nn::LayerNorm> {
    let weight = vb.get(size, "weight")?.dequantize(vb.device())?;
    let bias = vb.get(size, "bias")?.dequantize(vb.device())?;
    Ok(callosum_nn::LayerNorm::new(weight, bias, eps))
}

pub fn layer_norm_no_bias(size: usize, eps: f64, vb: VarBuilder) -> Result<callosum_nn::LayerNorm> {
    let weight = vb.get(size, "weight")?.dequantize(vb.device())?;
    Ok(callosum_nn::LayerNorm::new_no_bias(weight, eps))
}
...
impl Module for RmsNorm {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        let _enter = self.span.enter();
        callosum_nn::ops::rms_norm(x, &self.weight, self.eps as f32)
    }
}
