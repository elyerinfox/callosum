...
use callosum::{DType, Device, Module, Result, Tensor, D};
use callosum_nn::{linear_b as linear, Activation, Linear, VarBuilder};
...
impl Config {
    fn hidden_act(&self) -> Result<Activation> {
        match (self.hidden_act, self.hidden_activation) {
            ...
            (Some(_), Some(_)) => callosum::bail!("both hidden_act and hidden_activation are set"),
            (None, None) => callosum::bail!("none of hidden_act and hidden_activation are set"),
        }
    }
}
...
#[derive(Debug, Clone)]
pub struct Model {
    embed_tokens: callosum_nn::Embedding,
...
impl Model {
    ...
    pub fn embed_tokens(&self) -> &callosum_nn::Embedding {
        &self.embed_tokens
    }
