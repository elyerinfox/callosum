...
//!   --package callosum-examples \
//!   --example depth_anything_v2 \
//!   -- --color-map \
//!   --image callosum-examples/examples/yolo-v8/assets/bike.jpg
//! ```
//!
...
//!
//! <div align=center>
//!   <img src="https://github.com/huggingface/candle/raw/main/callosum-examples/examples/yolo-v8/assets/bike.jpg" alt="" width=640>
//! </div>
//!
//! ```bash
//! cargo run \
...
use callosum::{IndexOp, Result, Tensor, D};
use callosum_nn::{layer_norm, LayerNorm, Linear, Module, VarBuilder};
...
fn linear(vb: VarBuilder, in_dim: usize, out_dim: usize, bias: bool) -> Result<Linear> {
    if bias {
        callosum_nn::linear(in_dim, out_dim, vb)
    } else {
        callosum_nn::linear_no_bias(in_dim, out_dim, vb)
    }
}
...
impl Module for Attention {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let attn = callosum_nn::ops::softmax(&q.matmul(&k.t()?)?, D::Minus1)?;
...
impl PatchEmbed {
    fn new(
        ...
        let config = callosum_nn::Conv2dConfig {
            stride: patch_size,
            ..Default::default()
        };
        let proj = callosum_nn::conv2d(in_chans, embed_dim, patch_size, config, vb.pp("proj"))?;
...
impl Module for PatchEmbed {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        if (h % patch_h) != 0 {
            callosum::bail!("image height {h} is not a multiple of patch height {patch_h}")
        }
        if (w % patch_w) != 0 {
            callosum::bail!("image width {w} is not a multiple of patch width {patch_w}")
        }
