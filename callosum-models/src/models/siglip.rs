...
use callosum::{IndexOp, Module, Result, Tensor, D};
use callosum_nn::{layer_norm, linear, LayerNorm, Linear, VarBuilder};
...
fn default_text_hidden_act() -> callosum_nn::Activation {
    callosum_nn::Activation::GeluPytorchTanh
}
...
pub struct TextConfig {
    ...
    pub hidden_act: callosum_nn::Activation,
...
fn default_vision_hidden_act() -> callosum_nn::Activation {
    callosum_nn::Activation::GeluPytorchTanh
}
...
trait TransformerConfig {
    ...
    fn hidden_act(&self) -> callosum_nn::Activation;
}
...
impl VisionConfig {
    pub fn paligemma_3b_224() -> Self {
        Self {
            ...
            // Default values.
            num_channels: 3,
            hidden_act: callosum_nn::Activation::GeluPytorchTanh,
            layer_norm_eps: 1e-6,
        }
    }
...
impl MultiheadAttention {
    ...
    fn forward(&self, q: &Tensor, k: &Tensor, v: &Tensor) -> Result<Tensor> {
        ...
        let attn = callosum_nn::ops::softmax_last_dim(&attn)?;
...
impl Attention {
    ...
    fn forward(&self, xs: &Tensor, attention_mask: Option<&Tensor>) -> Result<Tensor> {
        ...
        // The original implementation upcasts to f32 but callosum_nn::ops::softmax should handle this properly.
...
struct Mlp {
    fc1: Linear,
    fc2: Linear,
    activation_fn: callosum_nn::Activation,
}
...
impl Mlp {
    fn new<C: TransformerConfig>(cfg: &C, vb: VarBuilder) -> Result<Self> {
        ...
        let fc1 = callosum_nn::linear(hidden_size, intermediate_size, vb.pp("fc1"))?;
        let fc2 = callosum_nn::linear(intermediate_size, hidden_size, vb.pp("fc2"))?;
        Ok(Self {
            fc1,
            fc2,
...
impl Module for Mlp {
    fn forward(&self, xs: &callosum::Tensor) -> Result<callosum::Tensor> {
        xs.apply(&self.fc1)?
...
struct VisionEmbeddings {
    patch_embedding: callosum_nn::Conv2d,
...
impl VisionEmbeddings {
    fn new(cfg: &VisionConfig, vb: VarBuilder) -> Result<Self> {
        let conv2d_cfg = callosum_nn::Conv2dConfig {
            stride: cfg.patch_size,
            ..Default::default()
        };
        let patch_embedding = callosum_nn::conv2d(
            cfg.num_channels,
            cfg.hidden_size,
            cfg.patch_size,
            conv2d_cfg,
        ...
        let embedder = callosum_nn::embedding(
...
impl TextEmbeddings {
    fn new(cfg: &TextConfig, vb: VarBuilder) -> Result<Self> {
        let token_embedding =
            callosum_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb.pp("token_embedding"))?;
