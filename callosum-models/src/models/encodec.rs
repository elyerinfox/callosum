...
use callosum::{DType, IndexOp, Layout, Module, Result, Shape, Tensor, D};
use callosum_nn::{conv1d, Conv1d, ConvTranspose1d, VarBuilder};
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            // This should be PadMode::Reflect which is currently unsupported in callosum.
...
fn pad1d(xs: &Tensor, pad_l: usize, pad_r: usize, mode: PadMode) -> Result<Tensor> {
    match mode {
        ...
        PadMode::Reflect => callosum::bail!("pad-mode 'reflect' is not supported"),
...
pub fn conv1d_weight_norm(
    ...
    out_c: usize,
    kernel_size: usize,
    config: callosum_nn::Conv1dConfig,
    vb: VarBuilder,
) -> Result<Conv1d> {
...
pub fn conv_transpose1d_weight_norm(
    ...
    kernel_size: usize,
    bias: bool,
    config: callosum_nn::ConvTranspose1dConfig,
    vb: VarBuilder,
...
impl callosum::CustomOp2 for CodebookEncode {
    ...
    fn cpu_fwd(
        &self,
        lhs_storage: &callosum::CpuStorage,
        lhs_layout: &Layout,
        rhs_storage: &callosum::CpuStorage,
        rhs_layout: &Layout,
    ) -> Result<(callosum::CpuStorage, Shape)> {
        use rayon::prelude::*;

        let (lhs_dim1, lhs_dim2) = lhs_layout.shape().dims2()?;
        let (rhs_dim1, rhs_dim2) = rhs_layout.shape().dims2()?;
        if lhs_dim2 != rhs_dim2 {
            callosum::bail!("CodebookEncode, mismatch on last dim, {lhs_layout:?} {rhs_layout:?}");
        }
        if lhs_dim2 == 0 {
            callosum::bail!("CodebookEncode, empty last dim {lhs_layout:?}")
        }
        let lhs = match lhs_layout.contiguous_offsets() {
            None => callosum::bail!("CodebookEncode, lhs has to be contiguous, got {lhs_layout:?}"),
            Some((o1, o2)) => {
        ...
        let rhs = match rhs_layout.contiguous_offsets() {
            None => callosum::bail!("CodebookEncode, rhs has to be contiguous, got {rhs_layout:?}"),
            Some((o1, o2)) => {
...
impl EuclideanCodebook {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        Ok(Self {
            inited,
            cluster_size,
            embed: callosum_nn::Embedding::new(embed, cfg.codebook_dim()),
            embed_avg,
            c2,
        })
    }
...
impl EncodecLSTM {
    pub fn new(dim: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        for layer_idx in 0..cfg.num_lstm_layers {
            let config = callosum_nn::LSTMConfig {
                layer_idx,
                ..Default::default()
            };
            let lstm = callosum_nn::lstm(dim, dim, config, vb.clone())?;
            layers.push(lstm)
        }
        Ok(Self { layers })
    }
...
impl EncodecConvTranspose1d {
    fn new(
        ...
        let cfg = callosum_nn::ConvTranspose1dConfig {
            stride,
            ..Default::default()
        };
...
pub struct EncodecConv1d {
    causal: bool,
    conv: Conv1d,
    norm: Option<callosum_nn::GroupNorm>,
    pad_mode: PadMode,
}
    ...
    pub fn new(
        ...
        let norm = match cfg.norm_type {
            ...
            NormType::TimeGroupNorm => {
                let gn = callosum_nn::group_norm(1, out_c, 1e-5, vb.pp("norm"))?;
                Some(gn)
            }
        };
        Ok(Self {
