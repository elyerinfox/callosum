...
use callosum_nn::{Conv2dConfig, Module, VarBuilder};
...
#[derive(Debug)]
struct Conv2dBN {
    c: callosum_nn::Conv2d,
    bn: callosum_nn::BatchNorm,
    span: tracing::Span,
}

impl Conv2dBN {
    fn new(in_: usize, out: usize, ks: usize, cfg: Conv2dConfig, vb: VarBuilder) -> Result<Self> {
        let c = callosum_nn::conv2d_no_bias(in_, out, ks, cfg, vb.pp("c"))?;
        let bn = callosum_nn::batch_norm(out, 1e-5, vb.pp("bn"))?;
...
impl MBConv {
    fn new(in_: usize, out: usize, expand_ratio: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let cfg2 = callosum_nn::Conv2dConfig {
            padding: 1,
            groups: hidden,
            ..Default::default()
        };
...
impl Mlp {
    fn new(in_: usize, hidden: usize, vb: VarBuilder) -> Result<Self> {
        let norm = callosum_nn::layer_norm(in_, 1e-5, vb.pp("norm"))?;
...
impl Attention {
    fn new(
        ...
        let h = dh + nh_kd * 2;
        let norm = callosum_nn::layer_norm(dim, 1e-5, vb.pp("norm"))?;
...
pub struct TinyViT {
    ...
    // norm_head: callosum_nn::LayerNorm,
    // head: callosum_nn::Linear,
...
impl TinyViT {
    pub fn new(
        ...
        let neck_conv2 = callosum_nn::conv2d_no_bias(256, 256, 3, cfg, vb.pp("neck.2"))?;
