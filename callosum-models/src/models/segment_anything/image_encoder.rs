use callosum::{DType, IndexOp, Result, Tensor};
use callosum_nn::{layer_norm, LayerNorm, Module, VarBuilder};

#[derive(Debug)]
struct PatchEmbed {
    proj: callosum_nn::Conv2d,
    span: tracing::Span,
}
...
impl PatchEmbed {
    fn new(
        ...
        let proj = callosum_nn::conv2d(in_chans, embed_dim, k_size, cfg, vb.pp("proj"))?;
...
// Ideally we would perform this operation in place but this is not supported in callosum at the
// moment. We should also investigate using f16 rather than f32.
struct Add3(usize, usize, usize, usize, usize);
impl callosum::CustomOp3 for Add3 {
    ...
    fn cpu_fwd(
        &self,
        s1: &callosum::CpuStorage,
        l1: &callosum::Layout,
        s2: &callosum::CpuStorage,
        l2: &callosum::Layout,
        s3: &callosum::CpuStorage,
        l3: &callosum::Layout,
    ) -> Result<(callosum::CpuStorage, callosum::Shape)> {
        use rayon::prelude::*;

        let Add3(b, q_h, q_w, k_h, k_w) = *self;
        let s1 = s1.as_slice::<f32>()?;
        let s1 = match l1.contiguous_offsets() {
            None => callosum::bail!("input1 has to be contiguous"),
            Some((o1, o2)) => &s1[o1..o2],
        };
        let s2 = s2.as_slice::<f32>()?;
        let s2 = match l2.contiguous_offsets() {
            None => callosum::bail!("input2 has to be contiguous"),
            Some((o1, o2)) => &s2[o1..o2],
        };
        let s3 = s3.as_slice::<f32>()?;
        let s3 = match l3.contiguous_offsets() {
            None => callosum::bail!("input3 has to be contiguous"),
            Some((o1, o2)) => &s3[o1..o2],
        };
        ...
        let dst = callosum::WithDType::to_cpu_storage_owned(dst);
...
impl Module for Attention {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let attn = {
            ...
            callosum_nn::ops::softmax_last_dim(&attn)?
        };
        let attn = {
            let _enter = self.span_matmul.enter();
            attn.matmul(&v)?
...
impl Block {
    fn new(
        ...
        let mlp = super::MlpBlock::new(dim, dim * 4, callosum_nn::Activation::Gelu, vb.pp("mlp"))?;
...
pub struct ImageEncoderViT {
    ...
    blocks: Vec<Block>,
    neck_conv1: callosum_nn::Conv2d,
    neck_ln1: super::LayerNorm2d,
    neck_conv2: callosum_nn::Conv2d,
