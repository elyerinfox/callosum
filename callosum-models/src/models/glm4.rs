...
impl CoreAttention {
    ...
    fn forward(
        ...
        let attention_probs = callosum_nn::ops::softmax_last_dim(&attention_scores)?;
...
impl SelfAttention {
    ...
    fn forward(
        ...
        if !self.multi_query_attention {
            callosum::bail!("only multi_query_attention=true is supported")
        }
...
impl Module for MLP {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        xs.apply(&self.dense_h_to_4h)?
            .apply(&callosum_nn::Activation::Swiglu)?
...
#[derive(Debug, Clone)]
struct Block {
    input_layernorm: callosum_nn::LayerNorm,
    self_attention: SelfAttention,
    post_attention_layernorm: callosum_nn::LayerNorm,
    mlp: MLP,
...
struct Transformer {
    layers: Vec<Block>,
    final_layernorm: Option<callosum_nn::LayerNorm>,
    rotary_emb: RotaryEmbedding,
}
...
#[derive(Debug, Clone)]
struct Embedding {
    word_embeddings: callosum_nn::Embedding,
    fp32_residual_connection: bool,
}

impl Embedding {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let word_embeddings = callosum_nn::embedding(
...
impl Module for Embedding {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        if self.fp32_residual_connection {
            xs.to_dtype(callosum::DType::F32)
        } else {
            xs.contiguous()
        }
    }
