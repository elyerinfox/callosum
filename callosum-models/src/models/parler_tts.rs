...
use callosum_nn::{layer_norm, linear_b as linear, Activation, LayerNorm, Linear, VarBuilder};
...
impl Attention {
    fn new(
        ...
        if cfg.rope_embeddings {
            callosum::bail!("rope embeddings are not supported");
        }
...
#[derive(Debug, Clone)]
pub struct Decoder {
    embed_tokens: Vec<callosum_nn::Embedding>,
...
impl Decoder {
    ...
    pub fn forward(
        ...
        if num_codebooks != self.num_codebooks {
            callosum::bail!("unexpected num codebooks in input {:?}", input_ids.shape())
        }
...
#[derive(Debug, Clone)]
pub struct Model {
    pub embed_prompts: callosum_nn::Embedding,
...
impl Model {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let embed_prompts = callosum_nn::embedding(
            cfg.vocab_size,
    ...
    pub fn generate(
        ...
        let all_audio_tokens = Tensor::new(all_audio_tokens, &callosum::Device::Cpu)?;
        Ok(all_audio_tokens)
    }
