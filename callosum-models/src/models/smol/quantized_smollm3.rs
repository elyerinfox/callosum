...
use callosum::quantized::gguf_file;
use callosum::{DType, Device, Module, Result, Tensor};
use callosum_nn::kv_cache::KvCache;
use callosum_nn::Activation;
use std::io::Write;
use std::sync::Arc;
    ...
    pub fn from_gguf(ct: &gguf_file::Content) -> Result<Self> {
        ...
        let get_u32 = |key: &str| -> Result<usize> {
            metadata
                ...
                .map(|v| v as usize)
                .ok_or_else(|| {
                    callosum::Error::Msg(format!("Missing or invalid metadata key: {}", key))
                })
        };
...
impl RmsNorm {
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let hidden_size = x.dim(callosum::D::Minus1)?;
        let x = x.to_dtype(internal_dtype)?;
        let norm_x = (x.sqr()?.sum_keepdim(callosum::D::Minus1)? / hidden_size as f64)?;
...
impl QuantizedAttention {
    fn new(
        ...
        use callosum::quantized::{GgmlDType, QTensor};
    ...
    fn forward(&mut self, x: &Tensor, mask: Option<&Tensor>, offset: usize) -> Result<Tensor> {
        ...
        attn_weights = callosum_nn::ops::softmax_last_dim(&attn_weights)?;
...
impl QuantizedModelForCausalLM {
    pub fn from_gguf<P: AsRef<std::path::Path>>(path: P, device: &Device) -> Result<Self> {
        ...
        let embed_tokens = callosum_nn::Embedding::new(embed_tensor_gpu, config.hidden_size);
