...
use callosum_nn::{kv_cache::KvCache, Embedding, RmsNorm};
...
impl QLinear {
    fn new<R: std::io::Read + std::io::Seek>(
        ...
        let inner = callosum::quantized::QMatMul::from_qtensor(w)?;
        Ok(Self { inner, span })
    }
}
...
impl LayerWeights {
    fn apply_rotary_emb(&self, xs: &Tensor, index_pos: usize) -> Result<Tensor> {
        ...
        callosum_nn::rotary_emb::rope(&xs.contiguous()?, &cos, &sin)
    }
