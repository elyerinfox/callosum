...
impl CustomOp1 for NonZero {
    ...
    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        ...
        let result = match storage {
            callosum::CpuStorage::U8(vs) => self.nonzero(vs, layout),
            callosum::CpuStorage::U32(vs) => self.nonzero(vs, layout),
            callosum::CpuStorage::I16(vs) => self.nonzero(vs, layout),
            callosum::CpuStorage::I32(vs) => self.nonzero(vs, layout),
            callosum::CpuStorage::I64(vs) => self.nonzero(vs, layout),
            callosum::CpuStorage::BF16(vs) => self.nonzero(vs, layout),
            callosum::CpuStorage::F16(vs) => self.nonzero(vs, layout),
            callosum::CpuStorage::F32(vs) => self.nonzero(vs, layout),
            callosum::CpuStorage::F64(vs) => self.nonzero(vs, layout),
...
impl NonZeroOp for Tensor {
    fn nonzero(&self) -> Result<Tensor> {
        if !self.is_contiguous() {
            return Err(callosum::Error::RequiresContiguous { op: "nonzero" });
        }
        let original_device = self.device();
        self.to_device(&callosum::Device::Cpu)?
...
impl DeepSeekV2RotaryEmbedding {
    ...
    pub fn new(cfg: &DeepSeekV2RopeConfig, dtype: DType, dev: &Device) -> Result<Self> {
        match &cfg.rope_scaling {
            Some(DeepSeekV2RopeScaling::LinearOrDynamic {
                scaling_type: _,
                factor: _,
            }) => callosum::bail!("linear and dynamic rope are not implemented yet!"),
    ...
    pub fn forward(
        ...
        let q_embed = callosum_nn::rotary_emb::rope_i(&q.contiguous()?, &cos, &sin)?;
        let k_embed = callosum_nn::rotary_emb::rope_i(&k.contiguous()?, &cos, &sin)?;
    ...
    fn new(
        ...
        let q = match cfg.q_lora_rank {
            Some(lora_rank) => {
                ...
                let b = callosum_nn::linear_no_bias(
                    lora_rank,
            ...
            None => QProj::Plain(callosum_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.num_attention_heads * q_head_dim,
                vb.pp("q_proj"),
            )?),
        ...
        let kv_a_proj_with_mqa = callosum_nn::linear_b(
            cfg.hidden_size,
        ...
        let kv_b_proj = callosum_nn::linear_no_bias(
            cfg.kv_lora_rank,
            cfg.num_attention_heads * (q_head_dim - cfg.qk_rope_head_dim + cfg.v_head_dim),
            vb.pp("kv_b_proj"),
        )?;

        let o_proj = callosum_nn::linear_b(
...
impl Mlp {
    fn new(
        ...
        Ok(Self {
            gate: callosum_nn::linear_no_bias(hidden_size, intermediate_size, vb.pp("gate_proj"))?,
            up: callosum_nn::linear_no_bias(hidden_size, intermediate_size, vb.pp("up_proj"))?,
            down: callosum_nn::linear_no_bias(intermediate_size, hidden_size, vb.pp("down_proj"))?,
            act: cfg.hidden_act,
        })
    }
...
impl MoeGate {
    ...
    fn forward(&self, xs: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        let scores = match self.cfg.scoring_func {
            ScoringFunc::Softmax => callosum_nn::ops::softmax_last_dim(&logits)?,
        };
...
impl DeepSeekV2 {
    pub fn new(cfg: &DeepSeekV2Config, vb: VarBuilder) -> Result<Self> {
        ...
        let lm_head = if !cfg.tie_word_embeddings {
            callosum_nn::linear_no_bias(cfg.hidden_size, cfg.vocab_size, vb.pp("lm_head"))?
        } else {
            callosum_nn::Linear::new(embed_tokens.embeddings().clone(), None)
        };
