...
use callosum::{bail, Context, DType, Device, Module, Result, Tensor, D};
use callosum_nn::{
...
pub struct DebertaV2Embeddings {
    ...
    embed_proj: Option<callosum_nn::Linear>,
}
...
impl DebertaV2Embeddings {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        let embed_proj: Option<callosum_nn::Linear> = if embedding_size != config.hidden_size {
            Some(callosum_nn::linear_no_bias(
                embedding_size,
                config.hidden_size,
                vb.pp("embed_proj"),
            )?)
...
impl XSoftmax {
    pub fn apply(input: &Tensor, mask: &Tensor, dim: D, device: &Device) -> Result<Tensor> {
        // NOTE: At the time of this writing, callosum does not have a logical-not operator.
        ...
        output = callosum_nn::ops::softmax(&output, dim)?;
...
pub struct DebertaV2DisentangledSelfAttention {
    ...
    query_proj: callosum_nn::Linear,
    key_proj: callosum_nn::Linear,
    value_proj: callosum_nn::Linear,
    dropout: StableDropout,
    device: Device,
    ...
    pos_ebd_size: isize,
    share_att_key: bool,
    pos_key_proj: Option<callosum_nn::Linear>,
    pos_query_proj: Option<callosum_nn::Linear>,
}
...
impl DebertaV2DisentangledSelfAttention {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        {
            return Err(callosum::Error::Msg(format!(
        ...
        let mut pos_key_proj: Option<callosum_nn::Linear> = None;
        let mut pos_query_proj: Option<callosum_nn::Linear> = None;
        ...
        if relative_attention {
            ...
            if !share_att_key {
                if config.pos_att_type.iter().any(|s| s == "c2p") {
                    pos_key_proj = Some(callosum_nn::linear(
                        config.hidden_size,
                        all_head_size,
                        vb.pp("pos_key_proj"),
                    )?);
                ...
                if config.pos_att_type.iter().any(|s| s == "p2c") {
                    pos_query_proj = Some(callosum_nn::linear(
                        config.hidden_size,
                        all_head_size,
...
impl DebertaV2SelfOutput {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let dense = callosum_nn::linear(config.hidden_size, config.hidden_size, vb.pp("dense"))?;
        let layer_norm = callosum_nn::layer_norm(
            config.hidden_size,
            config.layer_norm_eps,
            vb.pp("LayerNorm"),
        )?;
...
pub struct DebertaV2NERModel {
    ...
    dropout: callosum_nn::Dropout,
    classifier: callosum_nn::Linear,
}
...
impl DebertaV2NERModel {
    pub fn load(vb: VarBuilder, config: &Config, id2label: Option<Id2Label>) -> Result<Self> {
        ...
        let dropout = callosum_nn::Dropout::new(config.hidden_dropout_prob as f32);
        let classifier: callosum_nn::Linear = callosum_nn::linear_no_bias(
            config.hidden_size,
            id2label_len,
...
impl DebertaV2SeqClassificationModel {
    pub fn load(vb: VarBuilder, config: &Config, id2label: Option<Id2Label>) -> Result<Self> {
        ...
        let classifier = callosum_nn::linear(output_dim, id2label_len, vb.root().pp("classifier"))?;
...
pub(crate) fn make_log_bucket_position(
    ...
    let condition = lt_mid
        ...
        .mul(&gt_neg_mid.to_dtype(callosum::DType::F32)?)?
        .to_dtype(DType::U8)?;
