...
pub fn repeat_interleave<D: callosum::shape::Dim>(
    img: &Tensor,
    repeats: usize,
    dim: D,
) -> Result<Tensor> {
...
impl callosum::Module for EncoderBlock {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        xs.apply(&self.res1)?
            .apply(&self.res2)?
            .apply(&self.res3)?
...
impl callosum::Module for Encoder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
impl callosum::Module for Decoder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
struct VectorQuantizer {
    in_proj: Conv1d,
    out_proj: Conv1d,
    codebook: callosum_nn::Embedding,
    stride: usize,
}
...
impl VectorQuantizer {
    fn new(
        ...
        let codebook = callosum_nn::embedding(cb_size, cb_dim, vb.pp("codebook"))?;
        Ok(Self {
            in_proj,
            out_proj,
            codebook,
...
impl ResidualVectorQuantizer {
    ...
    fn from_codes(&self, codes: &[&Tensor]) -> Result<Tensor> {
        ...
        match sum {
            Some(s) => Ok(s),
            None => callosum::bail!("empty codebooks"),
        }
    }
}
