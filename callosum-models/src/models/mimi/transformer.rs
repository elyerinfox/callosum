...
use callosum::{DType, Device, IndexOp, Module, Result, StreamTensor, StreamingModule, Tensor, D};
use callosum_nn::{linear_no_bias, Linear, VarBuilder};
use std::sync::Arc;

fn linear(in_d: usize, out_d: usize, bias: bool, vb: VarBuilder) -> Result<Linear> {
    if bias {
        callosum_nn::linear(in_d, out_d, vb)
    } else {
...
pub struct Config {
    ...
    pub gating: Option<callosum_nn::Activation>,
...
impl RotaryEmbedding {
    ...
    pub fn apply_rotary_emb(&self, qk: &Tensor, seqlen_offset: usize) -> Result<Tensor> {
        ...
        callosum_nn::rotary_emb::rope_i(&qk.to_dtype(DType::F32)?, &c, &s)?.to_dtype(qk_dtype)
    }
}
...
pub struct StreamingMultiheadAttention {
    ...
    kv_cache: callosum_nn::kv_cache::RotatingKvCache,
    pos: usize,
    use_flash_attn: bool,
    span: tracing::Span,
}
...
impl StreamingMultiheadAttention {
    pub fn new(rope: &Option<Arc<RotaryEmbedding>>, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        Ok(Self {
            ...
            context: cfg.context,
            neg_inf,
            kv_cache: callosum_nn::kv_cache::RotatingKvCache::new(2, cfg.context),
            pos: 0,
    ...
    pub fn forward(&mut self, xs: &Tensor, mask: Option<&Tensor>) -> Result<Tensor> {
        let _enter = self.span.enter();
        if self.kv_repeat != 1 {
            callosum::bail!("only kv-repeat = 1 is supported")
        }
        let (b, t, hd) = xs.dims3()?;
        ...
        let xs = if q.dtype() == DType::BF16 && self.use_flash_attn {
            ...
            let ws = callosum_nn::ops::softmax_last_dim(&pre_ws)?; // b,h,t,k
            ws.matmul(&v)? // b,h,t,d
        };
        let xs = xs
    ...
    pub fn set_kv_cache(&mut self, kv_cache: callosum_nn::kv_cache::RotatingKvCache) {
...
pub enum Mlp {
    ...
    Gating {
        linear_in: Linear,
        linear_out: Linear,
        activation: callosum_nn::Activation,
        span: tracing::Span,
    },
}
...
impl Module for RmsNorm {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        callosum_nn::ops::rms_norm(xs, &self.alpha, self.eps)
    }
}
    ...
    pub fn new(d_model: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let norm = match cfg.norm {
            super::NormType::LayerNorm => {
                let norm = callosum_nn::layer_norm(d_model, 1e-5, vb)?;
                Self::LayerNorm(norm)
            }
...
pub struct StreamingTransformerLayer {
    ...
    cross_attn: Option<(callosum_nn::LayerNorm, StreamingMultiheadCrossAttention)>,
    norm_first: bool,
    span: tracing::Span,
}
...
impl StreamingTransformerLayer {
    pub fn new(rope: &Option<Arc<RotaryEmbedding>>, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        if cfg.use_conv_block {
            callosum::bail!("conv-block is not supported")
        }
        let d_model = cfg.d_model;
        let mlp = Mlp::new(cfg, vb.clone())?;
        let (norm1, norm2) = match cfg.norm {
            super::NormType::LayerNorm => {
                let norm1 = callosum_nn::layer_norm(d_model, 1e-5, vb.pp("input_layernorm"))?;
                let norm2 =
                    callosum_nn::layer_norm(d_model, 1e-5, vb.pp("post_attention_layernorm"))?;
        ...
        let cross_attn = if cfg.cross_attention {
            let norm_cross = callosum_nn::layer_norm(cfg.d_model, 1e-5, vb.pp("norm_cross"))?;
    ...
    pub fn forward(
        ...
        let _enter = self.span.enter();
        if !self.norm_first {
            callosum::bail!("only norm_first = true is supported")
        }
...
impl StreamingTransformer {
    ...
    pub fn copy_state(&mut self, from: &Self) -> Result<()> {
        if self.layers.len() != from.layers.len() {
            callosum::bail!("cannot copy kv-caches as the transformers have different depths")
        }
        self.layers
            .iter_mut()
...
fn flash_attn(
    ...
    callosum_flash_attn::flash_attn(q, k, v, softmax_scale, causal)
}
