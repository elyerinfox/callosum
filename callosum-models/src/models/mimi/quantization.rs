...
use callosum::{IndexOp, Layout, Result, Shape, Tensor, D};
use callosum_nn::{linear, Linear, VarBuilder};
...
impl EuclideanCodebook {
    ...
    pub fn decode(&self, indexes: &Tensor) -> Result<Tensor> {
        ...
        // let ys = callosum_nn::Embedding::new(self.embedding.clone(), self.dim).forward(xs)?;
...
impl ResidualVectorQuantization {
    ...
    pub fn decode(&self, xs: &Tensor) -> Result<Tensor> {
        if self.layers.is_empty() {
            callosum::bail!("empty layers in ResidualVectorQuantization")
        }
...
pub struct ResidualVectorQuantizer {
    ...
    input_proj: Option<callosum_nn::Conv1d>,
    output_proj: Option<callosum_nn::Conv1d>,
}
...
impl ResidualVectorQuantizer {
    pub fn new(
        ...
        let input_proj = if input_dim == dim && !force_projection {
            ...
            let c = callosum_nn::conv1d_no_bias(
                input_dim,
                dim,
                1,
                Default::default(),
