...
use callosum::{Context, Module, ModuleT, Result, Tensor, D};
use callosum_nn::{conv2d_no_bias, layer_norm, Activation, Conv2dConfig, VarBuilder};
...
struct SegformerOverlapPatchEmbeddings {
    projection: Conv2d,
    layer_norm: callosum_nn::LayerNorm,
}
...
impl SegformerOverlapPatchEmbeddings {
    fn new(
        ...
        let layer_norm =
            callosum_nn::layer_norm(hidden_size, config.layer_norm_eps, vb.pp("layer_norm"))?;
        Ok(Self {
            projection,
            layer_norm,
        })
...
impl Module for SegformerEfficientSelfAttention {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let attention_scores = callosum_nn::ops::softmax_last_dim(&attention_scores)?;
...
struct SegformerLayer {
    layer_norm_1: callosum_nn::LayerNorm,
    attention: SegformerAttention,
    layer_norm_2: callosum_nn::LayerNorm,
    mlp: SegformerMixFFN,
}
...
struct SegformerEncoder {
    ...
    layer_norms: Vec<callosum_nn::LayerNorm>,
}
...
struct SegformerDecodeHead {
    ...
    linear_fuse: callosum_nn::Conv2d,
    batch_norm: callosum_nn::BatchNorm,
    classifier: callosum_nn::Conv2d,
}
...
impl SegformerDecodeHead {
    fn new(config: &Config, num_labels: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let batch_norm = callosum_nn::batch_norm(
            config.decoder_hidden_size,
            config.layer_norm_eps,
            vb.pp("batch_norm"),
        )?;
