...
impl BertSelfAttention {
    ...
    fn forward(&self, xs: &Tensor, bias: &Tensor) -> Result<Tensor> {
        ...
        let attention_probs = {
            ...
            callosum_nn::ops::softmax_last_dim(&attention_scores)?
        };
...
impl BertGLUMLP {
    fn new(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        let act = callosum_nn::Activation::Gelu; // geglu
...
impl BertEncoder {
    fn new(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        if cfg.position_embedding_type != PositionEmbeddingType::Alibi {
            callosum::bail!("only alibi is supported as a position-embedding-type")
        }
