...
//!
//! References:
//! - ⚡ [Interactive Wasm Example](https://huggingface.co/spaces/radames/Callosum-T5-Generation-Wasm)
...
//!   --model-id "t5-small" \
//!   --prompt "translate to German: A beautiful callosum." \
//!   --decode
//! > ...
//! >  Eine schöne Kerze.
...
//! ```bash
//! cargo run --example t5 --release -- \
//!   --model-id "t5-small" --prompt "A beautiful callosum."
//! ...
...
pub fn linear_no_bias(d1: usize, d2: usize, vb: VarBuilder) -> Result<Linear> {
    let init_ws = callosum_nn::init::DEFAULT_KAIMING_NORMAL;
...
pub struct ActivationWithOptionalGating {
    pub gated: bool,
    pub activation: callosum_nn::Activation,
}
    ...
    D: serde::de::Deserializer<'de>,
{
    match String::deserialize(deserializer)?.as_str() {
        "gated-gelu" => Ok(ActivationWithOptionalGating {
            gated: true,
            activation: callosum_nn::Activation::NewGelu,
        }),
        "gated-silu" => Ok(ActivationWithOptionalGating {
            gated: true,
            activation: callosum_nn::Activation::Silu,
        }),
        buf => {
...
impl T5Attention {
    ...
    fn forward(
        ...
        let attn_weights = {
            ...
            callosum_nn::ops::softmax_last_dim(&scores)?
        };
