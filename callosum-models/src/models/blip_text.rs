...
//!
//! - ⚡ [Interactive Wasm Example](https://huggingface.co/spaces/radames/Callosum-BLIP-Image-Captioning)
...
use callosum_nn::{layer_norm, LayerNorm, VarBuilder};
...
struct TextIntermediate {
    dense: Linear,
    intermediate_act_fn: callosum_nn::Activation,
}
...
impl TextLayer {
    ...
    fn forward(
        ...
        let attention_output = match &mut self.cross_attention {
            ...
            None => callosum::bail!("expected some cross-attn"),
        };
...
struct TextPredictionHeadTransform {
    dense: Linear,
    transform_act_fn: callosum_nn::Activation,
    layer_norm: LayerNorm,
}
