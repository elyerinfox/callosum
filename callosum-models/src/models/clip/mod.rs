...
pub struct ClipModel {
    ...
    vision_model: ClipVisionTransformer,
    visual_projection: callosum_nn::Linear,
    text_projection: callosum_nn::Linear,
    logit_scale: Tensor,
}
...
impl ClipModel {
    pub fn new(vs: callosum_nn::VarBuilder, c: &ClipConfig) -> Result<Self> {
        ...
        let visual_projection = callosum_nn::linear_no_bias(
            c.vision_config.embed_dim,
            c.vision_config.projection_dim,
            vs.pp("visual_projection"),
        )?;
        let text_projection = callosum_nn::linear_no_bias(
