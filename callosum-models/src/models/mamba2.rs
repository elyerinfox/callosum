...
use callosum_nn::{RmsNorm, VarBuilder};
...
impl Mamba2Block {
    pub fn new(layer_idx: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let norm = callosum_nn::rms_norm(d_inner, 1e-5, vb.pp("norm"))?;
    ...
    pub fn forward(&self, xs: &Tensor, state: &mut State) -> Result<Tensor> {
        ...
        let xbc_conv = callosum_nn::ops::silu(&xbc_conv)?;
        ...
        let y = (y * callosum_nn::ops::silu(&z)?)?;
...
impl ResidualBlock {
    pub fn new(layer_idx: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let norm = callosum_nn::rms_norm(cfg.d_model, 1e-5, vb.pp("norm"))?;
...
#[derive(Clone, Debug)]
pub struct Model {
    embedding: callosum_nn::Embedding,
...
impl Model {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let embedding = callosum_nn::embedding(cfg.vocab_size(), cfg.d_model, vb.pp("embeddings"))?;
        let mut layers = Vec::with_capacity(cfg.n_layer);
        let vb_l = vb.pp("layers");
        for layer_idx in 0..cfg.n_layer {
            layers.push(ResidualBlock::new(layer_idx, cfg, vb_l.pp(layer_idx))?);
        }
        let norm_f = callosum_nn::rms_norm(cfg.d_model, 1e-5, vb.pp("norm_f"))?;
