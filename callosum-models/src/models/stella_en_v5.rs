...
use callosum::{DType, Device, Error, IndexOp, Module, Result, Tensor, D};
use callosum_nn::{layer_norm, Activation, LayerNorm, VarBuilder};
use std::sync::Arc;
...
impl Config {
    ...
    pub fn new_1_5_b_v5(embed_dim: EmbedDim) -> Self {
        ...
        Self {
            ...
            activation_fn: callosum_nn::Activation::Silu,
            vocab_size: 151646,
            hidden_size: 1536,
...
impl RotaryEmbedding {
    ...
    fn apply_rotary_emb_qkv(&self, q: &Tensor, k: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        let q_embed = callosum_nn::rotary_emb::rope(&q.contiguous()?, &cos, &sin)?;
        let k_embed = callosum_nn::rotary_emb::rope(&k.contiguous()?, &cos, &sin)?;
        Ok((q_embed, k_embed))
    }
}
        ...
        match self.variant {
            ModelVariant::Large => {
                    ...
                    (&self.post_attention_layernorm, &self.layernorm)
                {
                    ...
                    return Err(callosum::error::Error::Msg(
...
pub struct Embeddings {
    ...
    embeddings: callosum_nn::Embedding,
    // following are specifically for 400M
    token_type_embeddings: Option<callosum_nn::Embedding>,
...
impl Embeddings {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let (embeddings, token_type_embeddings, layer_norm, position_ids) = match cfg.variant {
            ModelVariant::Large => (
                callosum_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb.pp("embed_tokens"))?,
                None,
                None,
                None,
            ),
            ModelVariant::Small => {
                ...
                let weight = vb.pp("LayerNorm").get_with_hints(
                    cfg.hidden_size,
                    "weight",
                    callosum_nn::Init::Const(1.0),
                )?;
                let bias = vb.pp("LayerNorm").get_with_hints(
                    cfg.hidden_size,
                    "bias",
                    callosum_nn::Init::Const(0.0),
                )?;
                let dev = bias.device().clone();

                let layer_norm = callosum_nn::LayerNorm::new(weight, bias, cfg.norm_eps);

                (
                    callosum_nn::embedding(
                        cfg.vocab_size,
                        cfg.hidden_size,
...
impl Model {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        // let embed_tokens =
        //     callosum_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb_m.pp("embed_tokens"))?;
