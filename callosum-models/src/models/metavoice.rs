...
use callosum::{DType, Device, Error as E, IndexOp, Module, Result, Tensor, D};
use callosum_nn::{embedding, linear_b, rms_norm, Embedding, Linear, RmsNorm, VarBuilder};
...
pub mod speaker_encoder {
    ...
    impl Model {
        pub fn new(cfg: Config, vb: VarBuilder) -> Result<Self> {
            ...
            for layer_idx in 0..cfg.model_num_layers {
                let c = callosum_nn::LSTMConfig {
                    layer_idx,
                    ..Default::default()
                };
                let lstm = callosum_nn::lstm(
                    cfg.mel_n_channels,
        ...
        pub fn embed_utterance(
            ...
            let max_wave_length = match wav_slices.last() {
                Some(v) => v.1,
                None => callosum::bail!("empty wav slices"),
            };
    ...
    impl Module for Model {
        fn forward(&self, xs: &Tensor) -> Result<Tensor> {
            ...
            // This is different from the Python transformers version as callosum LSTM is batch first.
            let xs = xs.t()?;
            let mut xs = xs.clone();
    ...
    impl BPE {
        pub fn from_json(json: &serde_json::Value, end_of_text: usize) -> Result<Self> {
            let json = match json.as_object() {
                None => callosum::bail!("json value is not an object"),
                Some(json) => json,
            };
            let re = match json.get("pat_str") {
                None => callosum::bail!("json object has no pat_str field"),
                Some(pat_str) => match pat_str.as_str() {
                    None => callosum::bail!("pat_str field is not a string"),
                    Some(pat_str) => fancy_regex::Regex::new(pat_str).map_err(E::wrap)?,
                },
            };
            let offset = match json.get("offset") {
                None => callosum::bail!("json object has no offset field"),
                Some(offset) => match offset.as_u64() {
                    None => callosum::bail!("offset field is not a positive int"),
            ...
            let mergeable_ranks = match json.get("mergeable_ranks") {
                None => callosum::bail!("json object has no mergeable_ranks field"),
                Some(mr) => match mr.as_object() {
                    None => callosum::bail!("mergeable_ranks is not an object"),
                    Some(mr) => mr,
                },
            };
            for (key, value) in mergeable_ranks.iter() {
                let value = match value.as_u64() {
                    None => callosum::bail!("mergeable_ranks '{key}' is not a u64"),
...
pub mod gpt {
    ...
    enum Norm {
        RMSNorm(callosum_nn::RmsNorm),
        LayerNorm(callosum_nn::LayerNorm),
    }
    ...
    impl Norm {
        fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
            match cfg.norm_type {
                NormType::RMSNorm => {
                    let rms_norm = callosum_nn::rms_norm(cfg.n_embd, cfg.rmsnorm_eps, vb)?;
                    Ok(Self::RMSNorm(rms_norm))
                }
                NormType::LayerNorm => {
                    let ln_cfg = callosum_nn::LayerNormConfig {
                        affine: cfg.bias,
                        ..Default::default()
                    };
                    let layer_norm = callosum_nn::layer_norm(cfg.n_embd, ln_cfg, vb)?;
    ...
    impl SelfAttention {
        fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
            ...
            if cfg.attn_kernel_type != AttnKernelType::TorchAttn {
                callosum::bail!("only TorchAttn is supported")
            }
            if cfg.kv_cache_enabled {
                callosum::bail!("kv_cache_enabled=true is not supported")
            }
    ...
    impl Module for SelfAttention {
        fn forward(&self, xs: &Tensor) -> Result<Tensor> {
            ...
            // TODO: causal mask
            let att = callosum_nn::ops::softmax_last_dim(&att)?;
            ...
            let slf = match cfg.nonlinearity_type {
                ...
                NonLinearityType::Swiglu => {
                    ...
                    let swiglu_multiple_of = match cfg.swiglu_multiple_of {
                        None => callosum::bail!("swiglu-multiple-of has to be set"),
                        Some(smo) => smo,
                    };
    ...
    #[allow(clippy::upper_case_acronyms)]
    pub struct Model {
        wtes: Vec<callosum_nn::Embedding>,
        wpe: callosum_nn::Embedding,
        h: Vec<Block>,
        ln_f: Norm,
        lm_heads: Vec<Linear>,
        cfg: Config,
    ...
    impl Model {
        pub fn new(cfg: Config, vb: VarBuilder) -> Result<Self> {
            ...
            for (idx, vocab_size) in cfg.vocab_sizes.iter().enumerate() {
                let wte = callosum_nn::embedding(*vocab_size, cfg.n_embd, vb_w.pp(idx))?;
                wtes.push(wte)
            }
            let wpe = callosum_nn::embedding(cfg.block_size, cfg.n_embd, vb_t.pp("wpe"))?;
...
pub mod transformer {
    ...
    impl Module for FeedForward {
        fn forward(&self, xs: &Tensor) -> Result<Tensor> {
            let _enter = self.span.enter();
            let swiglu = (callosum_nn::ops::silu(&xs.apply(&self.w1)?)? * xs.apply(&self.w3))?;
    ...
    impl Attention {
        ...
        fn forward(&mut self, xs: &Tensor, _pos: usize, mask: &Tensor) -> Result<Tensor> {
            ...
            let attn_weights = callosum_nn::ops::softmax_last_dim(&attn_weights)?;
