...
use callosum_nn::{Activation, VarBuilder};
use std::sync::Arc;
...
#[derive(Debug, Clone)]
struct Mlp {
    gate_proj: callosum_nn::Linear,
    up_proj: callosum_nn::Linear,
    down_proj: callosum_nn::Linear,
    act_fn: Activation,
}
...
impl Mlp {
    fn new(cfg: &TextEncoderConfig, vb: VarBuilder) -> Result<Self> {
        Ok(Self {
            gate_proj: callosum_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.intermediate_size,
                vb.pp("gate_proj"),
            )?,
            up_proj: callosum_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.intermediate_size,
                vb.pp("up_proj"),
            )?,
            down_proj: callosum_nn::linear_no_bias(
                cfg.intermediate_size,
                cfg.hidden_size,
                vb.pp("down_proj"),
            )?,
...
impl Attention {
    ...
    fn forward(&self, x: &Tensor, attn_mask: Option<&Tensor>, offset: usize) -> Result<Tensor> {
        ...
        let probs = callosum_nn::ops::softmax_last_dim(&scores)?;
...
impl ZImageTextEncoder {
    pub fn new(cfg: &TextEncoderConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let embed_tokens =
            callosum_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb_model.pp("embed_tokens"))?;
    ...
    pub fn forward(&self, input_ids: &Tensor) -> Result<Tensor> {
        ...
        callosum::bail!("Layer index out of bounds")
    }
