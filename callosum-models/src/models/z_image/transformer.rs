...
use callosum::{DType, Device, IndexOp, Module, Result, Tensor, D};
use callosum_nn::{linear, linear_no_bias, VarBuilder};
...
fn flash_attn(_: &Tensor, _: &Tensor, _: &Tensor, _: f32, _: bool) -> Result<Tensor> {
    callosum::bail!("flash-attn feature not enabled, compile with '--features flash-attn'")
}
...
pub struct TimestepEmbedder {
    linear1: callosum_nn::Linear,
    linear2: callosum_nn::Linear,
...
pub struct FeedForward {
    w1: callosum_nn::Linear,
    w2: callosum_nn::Linear,
    w3: callosum_nn::Linear,
}
...
pub struct ZImageAttention {
    to_q: callosum_nn::Linear,
    to_k: callosum_nn::Linear,
    to_v: callosum_nn::Linear,
    to_out: callosum_nn::Linear,
...
impl ZImageAttention {
    ...
    fn attention_basic(
        ...
        let attn_probs = callosum_nn::ops::softmax_last_dim(&attn_weights)?;
        attn_probs.matmul(v)
    }
...
pub struct ZImageTransformerBlock {
    ...
    ffn_norm1: RmsNorm,
    ffn_norm2: RmsNorm,
    adaln_modulation: Option<callosum_nn::Linear>,
}
...
pub struct FinalLayer {
    ...
    linear: callosum_nn::Linear,
    adaln_silu: callosum_nn::Linear,
}

impl FinalLayer {
    pub fn new(hidden_size: usize, out_channels: usize, vb: VarBuilder) -> Result<Self> {
        let norm_final = LayerNormNoParams::new(1e-6);
        let linear = callosum_nn::linear(hidden_size, out_channels, vb.pp("linear"))?;
        let adaln_dim = hidden_size.min(ADALN_EMBED_DIM);
        let adaln_silu =
            callosum_nn::linear(adaln_dim, hidden_size, vb.pp("adaLN_modulation").pp("1"))?;
...
pub struct ZImageTransformer2DModel {
    ...
    cap_embedder_linear: callosum_nn::Linear,
    x_embedder: callosum_nn::Linear,
