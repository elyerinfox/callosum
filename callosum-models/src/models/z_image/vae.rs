...
use callosum::{Module, Result, Tensor, D};
use callosum_nn::{conv2d, group_norm, Conv2d, Conv2dConfig, GroupNorm, VarBuilder};
...
fn scaled_dot_product_attention(q: &Tensor, k: &Tensor, v: &Tensor) -> Result<Tensor> {
    ...
    callosum_nn::ops::softmax_last_dim(&attn_weights)?.matmul(v)
}
...
impl Attention {
    fn new(channels: usize, num_groups: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let to_q = callosum_nn::linear(channels, channels, vb.pp("to_q"))?;
        let to_k = callosum_nn::linear(channels, channels, vb.pp("to_k"))?;
        let to_v = callosum_nn::linear(channels, channels, vb.pp("to_v"))?;
        let to_out = callosum_nn::linear(channels, channels, vb.pp("to_out").pp("0"))?;
        Ok(Self {
            group_norm,
            to_q,
            to_k,
...
impl Module for ResnetBlock2D {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let h = xs
            .apply(&self.norm1)?
            .apply(&callosum_nn::Activation::Swish)?
            .apply(&self.conv1)?
            .apply(&self.norm2)?
            .apply(&callosum_nn::Activation::Swish)?
            .apply(&self.conv2)?;
