...
use callosum::{ModuleT, Result, Tensor};
use callosum_nn::{FuncT, VarBuilder};
...
fn fully_connected(
    ...
    let dropout1 = callosum_nn::Dropout::new(0.5);
    let dropout2 = callosum_nn::Dropout::new(0.5);
    let dropout3 = callosum_nn::Dropout::new(0.5);
    Ok(FuncT::new(move |xs, train| {
        ...
        let lin3 = callosum_nn::linear(4096, num_classes, vb.pp("head.fc"))?;
...
fn get_weights_and_biases(
    ...
    let init_bs = callosum_nn::Init::Uniform {
        lo: -bound,
        up: bound,
    };
    let bs = vs.get_with_hints(target_in, "bias", init_bs)?;
    Ok(callosum_nn::Linear::new(ws, Some(bs)))
}
