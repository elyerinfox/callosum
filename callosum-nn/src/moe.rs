...
#[cfg(feature = "cuda")]
use callosum::cuda_backend::kernels::ffi;
#[allow(unused_imports)]
use callosum::quantized::{self, QTensor};
use callosum::{Result, Tensor};
...
pub fn moe_gemm(
    ...
    use callosum::cuda_backend::cudarc::driver::DevicePtr;
    use callosum::DType;
    use half::{bf16, f16};

    fn cuda_fwd<
        T: callosum::cuda_backend::CudaDType + callosum::cuda_backend::cudarc::driver::DeviceRepr,
    >(
        input: &Tensor,
        weights: &Tensor,
        topk_weights: &Option<Tensor>,
        ...
        let data_type = match input.dtype() {
            ...
            DType::BF16 => 1,
            _ => {
                callosum::bail!("moe_gemm_wmma only accepts f16/bf16 inputs")
            }
        };

        let (input, _) = input.storage_and_layout();
        let input = match &*input {
            callosum::Storage::Cuda(c) => c.as_cuda_slice::<T>()?,
            _ => callosum::bail!("input must be a cuda tensor"),
        };

        let (weights, _) = weights.storage_and_layout();
        let weights = match &*weights {
            callosum::Storage::Cuda(c) => c.as_cuda_slice::<T>()?,
            _ => callosum::bail!("weight must be a cuda tensor"),
        };

        let (sorted_token_ids, _) = sorted_token_ids.storage_and_layout();
        let sorted_token_ids = match &*sorted_token_ids {
            callosum::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?,
            _ => callosum::bail!("sorted_token_ids must be a cuda tensor"),
        };

        let (experts_ids, _) = experts_ids.storage_and_layout();
        let experts_ids = match &*experts_ids {
            callosum::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?,
            _ => callosum::bail!("experts_ids must be a cuda tensor"),
        };
        ...
        let topk_weights_ptr = if let Some(topk_weights) = &topk_weights {
            ...
            let topk_weights = match &*topk_weights {
                callosum::Storage::Cuda(c) => c.as_cuda_slice::<f32>()?,
                _ => callosum::bail!("topk_weights must be a cuda tensor"),
            };
        ...
        use callosum::op::BackpropOp;
        let output = callosum::CudaStorage::wrap_cuda_slice(output, dev.clone());
        let output = Tensor::from_storage(
            callosum::Storage::Cuda(output),
            (size_m, size_n),
            BackpropOp::none(),
            false,
        );
    ...
    match input.dtype() {
        ...
        _ => {
            callosum::bail!("moe_gemm only accepts f16/bf16 inputs")
        }
    }
}
...
pub fn moe_gemm(
    ...
    callosum::bail!("moe_gemm is only implemented for the cuda backend")
}
...
pub fn moe_gemm_gguf(
    ...
    topk: usize,
    is_prefill: bool,
    dtype: callosum::DType,
) -> Result<Tensor> {
    use callosum::cuda_backend::cudarc::driver::DevicePtr;
    use callosum::quantized::GgmlDType;
    use callosum::DType;
    use half::{bf16, f16};
    ...
    match input.dtype() {
        ...
        _ => {
            callosum::bail!("moe_gemm_gguf only accepts f32 inputs")
        }
    }
}
...
pub fn moe_gemm_gguf(
    ...
    callosum::bail!("moe_gemm_gguf is only implemented for the cuda backend")
}
