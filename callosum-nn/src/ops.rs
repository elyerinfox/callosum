...
use callosum::{CpuStorage, DType, Layout, Module, Result, Shape, Tensor, D};
use rayon::prelude::*;
...
///
/// ```rust
/// use callosum::{Tensor, Device, test_utils::to_vec2_round};
/// let a = Tensor::new(&[[0f32, 1., 0., 1.], [-2., 2., 3., -3.]], &Device::Cpu)?;
/// let a = callosum_nn::ops::softmax(&a, 1)?;
/// assert_eq!(
///     to_vec2_round(&a, 4)?,
///     &[
...
/// # Ok::<(), callosum::Error>(())
/// ```
pub fn softmax<D: callosum::shape::Dim>(xs: &Tensor, dim: D) -> Result<Tensor> {
...
pub fn log_softmax<D: callosum::shape::Dim>(xs: &Tensor, d: D) -> Result<Tensor> {
...
impl callosum::CustomOp1 for Sigmoid {
    fn name(&self) -> &'static str {
        "sigmoid"
    }

    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        use callosum::backend::BackendStorage;

        fn fwd<T: num_traits::Float>(v: T) -> T {
            (v.neg().exp() + T::one()).recip()
        }

        // FIXME: using `callosum::map_dtype` causes compilation errors.
        let storage = match storage {
            CpuStorage::BF16(slice) => {
                CpuStorage::BF16(callosum::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F16(slice) => {
                CpuStorage::F16(callosum::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F32(slice) => {
                CpuStorage::F32(callosum::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F64(slice) => {
                CpuStorage::F64(callosum::cpu_backend::unary_map(slice, layout, fwd))
            }
            _ => Err(callosum::Error::UnsupportedDTypeForOp(
                storage.dtype(),
                self.name(),
            ))?,
        };
    ...
    fn cuda_fwd(
        &self,
        storage: &callosum::CudaStorage,
        layout: &Layout,
    ) -> Result<(callosum::CudaStorage, Shape)> {
        use callosum::backend::BackendStorage;
        use callosum::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchConfig, PushKernelArg, ValidAsZeroBits,
        };
        use callosum::cuda_backend::SlicePtrOrNull;
        use callosum::cuda_backend::{kernel_name, kernels, Map1, WrapErr};
        use callosum::{CudaDevice, WithDType};
        ...
        impl Map1 for S {
            fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
                ...
                callosum::builder_arg!(builder, el_count, dims.len());
    ...
    fn metal_fwd(
        &self,
        storage: &callosum::MetalStorage,
        layout: &Layout,
    ) -> Result<(callosum::MetalStorage, Shape)> {
        use callosum::backend::BackendStorage;
        use callosum::MetalError;
        ...
        let src = callosum_metal_kernels::BufferOffset {
            buffer: storage.buffer(),
            offset_in_bytes: layout.start_offset() * storage.dtype().size_in_bytes(),
        };

        if layout.is_contiguous() {
            use callosum_metal_kernels::unary::contiguous;
            let kernel_name = match dtype {
                DType::F16 => contiguous::sigmoid::HALF,
                DType::F32 => contiguous::sigmoid::FLOAT,
                DType::BF16 => contiguous::sigmoid::BFLOAT,
                dtype => {
                    callosum::bail!("Metal contiguous unary sigmoid {dtype:?} not implemented")
                }
            };
            callosum_metal_kernels::call_unary_contiguous(
                device.metal_device(),
                &encoder,
                device.kernels(),
                kernel_name,
            ...
            use callosum_metal_kernels::unary::strided;
            let kernel_name = match dtype {
                DType::F16 => strided::sigmoid::HALF,
                DType::F32 => strided::sigmoid::FLOAT,
                DType::BF16 => strided::sigmoid::BFLOAT,
                dtype => {
                    callosum::bail!("Metal strided unary sigmoid {dtype:?} not implemented")
                }
            };
            let dst = callosum_metal_kernels::BufferOffset::zero_offset(&buffer);
            callosum_metal_kernels::call_unary_strided(
                device.metal_device(),
                &encoder,
                device.kernels(),
                kernel_name,
        ...
        let new_storage = callosum::MetalStorage::new(buffer, device.clone(), el_count, dtype);
...
pub fn dropout(xs: &Tensor, drop_p: f32) -> Result<Tensor> {
    ...
    if !(0. ..1.).contains(&drop_p) {
        callosum::bail!("dropout probability has to be in [0, 1), got {drop_p}")
    }
...
impl callosum::ModuleT for Dropout {
...
impl callosum::CustomOp1 for SoftmaxLastDim {
    fn name(&self) -> &'static str {
        "softmax-last-dim"
    }

    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        fn softmax<T: callosum::WithDType + num_traits::Float>(
            src: &[T],
            layout: &Layout,
        ) -> Result<(CpuStorage, Shape)> {
            let src = match layout.contiguous_offsets() {
                None => callosum::bail!("input has to be contiguous"),
            ...
            let storage = callosum::WithDType::to_cpu_storage_owned(dst);
        ...
        match storage {
            ...
            _ => callosum::bail!("unsupported dtype for softmax {:?}", storage),
        }
    }
    ...
    fn cuda_fwd(
        ...
        impl Map1 for S {
            fn f<T: DeviceRepr + WithDType>(
                ...
                builder.arg(&src);
                builder.arg(&dst);
                callosum::builder_arg!(builder, n_cols as i32);
                // SAFETY: ffi.
                unsafe { builder.launch(cfg) }.w()?;
                Ok(dst)
            }
        ...
        let dev = storage.device();
        let slice = S.map(&storage.slice, dev, layout)?;
        let dst = callosum::cuda_backend::CudaStorage {
            slice,
            device: dev.clone(),
        };
        Ok((dst, layout.shape().clone()))
    ...
    fn metal_fwd(
        ...
        let name = match storage.dtype() {
            ...
            dtype => callosum::bail!("softmax-last-dim is not implemented for {dtype:?}"),
        };

        let n = layout.stride().len();
        if !(layout.is_contiguous() && layout.stride()[n - 1] == 1) {
            callosum::bail!("Non contiguous softmax-last-dim is not implemented");
        }

        let last_dim = layout.dims()[layout.shape().rank() - 1];
        let elem_count = layout.shape().elem_count();
        let output = device.new_buffer(elem_count, storage.dtype(), "softmax")?;
        callosum_metal_kernels::call_last_softmax(
            device.metal_device(),
            &encoder,
            kernels,
            name,
        ...
        let newstorage =
            callosum::MetalStorage::new(output, device.clone(), elem_count, storage.dtype());
...
impl callosum::CustomOp2 for RmsNorm {
    fn name(&self) -> &'static str {
        "rms-norm"
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let alpha = match alpha_layout.contiguous_offsets() {
                None => callosum::bail!("alpha has to be contiguous"),
        ...
        match (s1, s2) {
            ...
            _ => callosum::bail!("unsupported dtype for rmsnorm {:?}", s1.dtype()),
        }
    }
    ...
    fn cuda_fwd(
        &self,
        s1: &callosum::CudaStorage,
        l1: &Layout,
        s2: &callosum::CudaStorage,
        l2: &Layout,
    ) -> Result<(callosum::CudaStorage, Shape)> {
        use callosum::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchConfig, PushKernelArg,
        };
        use callosum::cuda_backend::{kernel_name, kernels, Map2, WrapErr};
        use callosum::{CudaDevice, WithDType};
        ...
        impl Map2 for S {
            fn f<T: DeviceRepr + WithDType>(
                ...
                builder.arg(&dst);
                builder.arg(&alpha);
                callosum::builder_arg!(builder, n_cols as i32, block_size as i32, self.eps);
                // SAFETY: ffi.
                unsafe { builder.launch(cfg) }.w()?;
                Ok(dst)
            }
    ...
    fn metal_fwd(
        &self,
        s1: &callosum::MetalStorage,
        l1: &Layout,
        s2: &callosum::MetalStorage,
        l2: &Layout,
    ) -> Result<(callosum::MetalStorage, Shape)> {
        use callosum::backend::BackendStorage;
        ...
        let name = match (s1.dtype(), s2.dtype()) {
            ...
            (dt1, dt2) => callosum::bail!("rmsnorm is not implemented for {dt1:?} {dt2:?}"),
        };

        if !(l1.is_contiguous() && l2.is_contiguous()) {
            callosum::bail!("Non contiguous rmsnorm is not implemented");
        }

        let last_dim = l1.dims()[l1.shape().rank() - 1];
        let elem_count = l1.shape().elem_count();
        let output = device.new_buffer(elem_count, s1.dtype(), "rmsnorm")?;
        callosum_metal_kernels::call_rms_norm(
            device.metal_device(),
            &encoder,
            kernels,
            name,
...
impl callosum::CustomOp3 for LayerNorm {
    fn name(&self) -> &'static str {
        "layer-norm"
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let beta = match beta_layout.contiguous_offsets() {
                None => callosum::bail!("beta has to be contiguous"),
    ...
    fn cuda_fwd(
        ...
        s2: &callosum::CudaStorage,
        l2: &Layout,
        s3: &callosum::CudaStorage,
        l3: &Layout,
    ) -> Result<(callosum::CudaStorage, Shape)> {
        use callosum::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchConfig, PushKernelArg,
        };
        use callosum::cuda_backend::{kernel_name, kernels, Map3, WrapErr};
        use callosum::{CudaDevice, WithDType};
    ...
    fn metal_fwd(
        ...
        let name = match (s1.dtype(), s2.dtype(), s3.dtype()) {
            ...
            (dt1, dt2, dt3) => {
                callosum::bail!("layernorm is not implemented for {dt1:?} {dt2:?} {dt3:?}")
            }
        };

        if !(l1.is_contiguous() && l2.is_contiguous() && l3.is_contiguous()) {
            callosum::bail!("Non contiguous layernorm is not implemented");
        }

        let last_dim = l1.dims()[l1.shape().rank() - 1];
        let elem_count = l1.shape().elem_count();
        let output = device.new_buffer(elem_count, s1.dtype(), "layernorm")?;
        callosum_metal_kernels::call_layer_norm(
            device.metal_device(),
            &encoder,
            kernels,
            name,
...
pub fn replication_pad2d(xs: &Tensor, pad: usize) -> Result<Tensor> {
    match pad {
        ...
        n => callosum::bail!("replication-pad with a size of {n} is not supported"),
    }
}
...
impl callosum::CustomOp3 for Sdpa {
    fn name(&self) -> &'static str {
        "metal-sdpa"
    }
    ...
    fn cpu_fwd(
        ...
        callosum::bail!("SDPA has no cpu impl")
    }

    #[cfg(feature = "metal")]
    fn metal_fwd(
        &self,
        q: &callosum::MetalStorage,
        q_l: &Layout,
        k: &callosum::MetalStorage,
        k_l: &Layout,
        v: &callosum::MetalStorage,
        v_l: &Layout,
    ) -> Result<(callosum::MetalStorage, Shape)> {
        use callosum::backend::BackendStorage;
        use callosum_metal_kernels::SdpaDType;
        ...
        if q_l.dim(D::Minus1)? != k_l.dim(D::Minus1)? {
            callosum::bail!("`q` and `k` last dims must match");
        }

        // k,v must have matching n kv heads
        if v_l.dim(D::Minus(3))? != k_l.dim(D::Minus(3))? {
            callosum::bail!("`k` and `v` head dims must match");
        }

        // n_heads % n_kv_heads == 0; n_heads >= 1, n_kv_heads >= 1.
        if q_l.dim(D::Minus(3))? % k_l.dim(D::Minus(3))? != 0 {
            callosum::bail!("query `n_heads` must be a multiple of `n_kv_heads`");
        }
        ...
        for t in [k.dtype(), v.dtype()] {
            if q.dtype() != t {
                callosum::bail!("all q, k, v dtypes must match.");
            }
        }
        ...
        let itype = match q.dtype() {
            ...
            other => callosum::bail!("unsupported sdpa type {other:?}"),
        };
        ...
        if supports_sdpa_vector {
            ...
            if k_seq >= TWO_PASS_K_THRESHOLD {
                let mut intermediate_shape = [
                    ...
                    &[callosum_metal_kernels::SDPA_2PASS_BLOCKS],
                ...
                callosum_metal_kernels::call_sdpa_vector_2pass(
                    q.device().device(),
                    &encoder,
                    q.device().kernels(),
                    q_l.start_offset(),
                ...
                encoder.set_label("vector_attention");
                callosum_metal_kernels::call_sdpa_vector(
                    q.device().device(),
                    &encoder,
                    q.device().kernels(),
                    q_l.start_offset(),
            ...
            if self.softcapping != 1. {
                callosum::bail!("SDPA full requires softcapping to be disabled (1.0)");
            }
            ...
            let (mask_type, mask_buffer, mask_strides) = if let Some(mask) = &self.mask {
                ...
                let mask_buffer = match &**mask_s {
                    callosum::Storage::Metal(m) => m.buffer(),
                    _ => callosum::bail!("Expected metal device for mask"),
                };
                ...
                if mask_type != itype {
                    callosum::bail!("Mask type {mask_type:?} must match q type {itype:?}");
                }
            ...
            callosum_metal_kernels::call_sdpa_full(
                q.device().device(),
                &encoder,
                q.device().kernels(),
                q_l.start_offset(),
            ...
            callosum::bail!("must be vector or full sdpa kernel");
        }

        let newstorage = callosum::MetalStorage::new(output, device.clone(), elem_count, q.dtype());
