...
use callosum::quantized::GgmlType;
use callosum::{CpuStorage, Device, Layout, Module, Result, Shape, Tensor, D};
...
impl callosum::CustomOp1 for Im2Col {
...
impl Benchmark for QMatMul {
    type PreProcessData = (callosum::quantized::QMatMul, Tensor);
    type RunResult = Tensor;
    fn preprocess() -> Result<Self::PreProcessData> {
        let zeros = vec![callosum::quantized::k_quants::BlockQ4_0::zeros(); 4096 * 11008 / 32];
        let mm = callosum::quantized::QTensor::new(
            callosum::quantized::QStorage::Cpu(Box::new(zeros)),
            (4096, 11008),
        )?;
        let mm = callosum::quantized::QMatMul::from_qtensor(mm)?;
...
impl Benchmark for Softmax {
    ...
    fn run_one(d: &Self::PreProcessData) -> Result<Self::RunResult> {
        callosum_nn::ops::softmax(d, D::Minus1)
    }
...
impl Benchmark for SoftmaxLastDim {
    ...
    fn run_one(d: &Self::PreProcessData) -> Result<Self::RunResult> {
        callosum_nn::ops::softmax_last_dim(d)
    }
