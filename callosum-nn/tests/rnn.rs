...
use callosum::{test_utils::to_vec2_round, DType, Device, Result, Tensor};
use callosum_nn::RNN;
...
fn lstm() -> Result<()> {
    ...
    .into_iter()
    .collect();
    let vb = callosum_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let lstm = callosum_nn::lstm(2, 3, Default::default(), vb)?;
...
fn gru() -> Result<()> {
    ...
    .collect();
    let vb = callosum_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let gru = callosum_nn::gru(2, 3, Default::default(), vb)?;
