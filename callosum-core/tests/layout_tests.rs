use callosum::{test_device, Device, IndexOp, Result, Tensor};
use callosum_core as callosum;
...
fn strided_blocks() -> Result<()> {
    use callosum::Device::Cpu;
    let tensor = Tensor::arange(0u32, 24u32, &Cpu)?.reshape((2, 3, 4))?;
    match tensor.strided_blocks() {
        callosum::StridedBlocks::SingleBlock { start_offset, len } => {
            assert_eq!(start_offset, 0);
            assert_eq!(len, 24);
        }
        callosum::StridedBlocks::MultipleBlocks { .. } => {
    ...
    let tensor = tensor.i((.., 1))?;
    match tensor.strided_blocks() {
        callosum::StridedBlocks::SingleBlock { .. } => {
            panic!("unexpected block structure")
        }
        callosum::StridedBlocks::MultipleBlocks {
            block_len,
