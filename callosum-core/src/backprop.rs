...
thread_local! {
    static CALLOSUM_GRAD_DO_NOT_DETACH: bool = {
        match std::env::var("CALLOSUM_GRAD_DO_NOT_DETACH") {
            Ok(s) => {
                !s.is_empty() && s != "0"
            },
            Err(_) => false,
...
impl Tensor {
    ...
    pub fn backward(&self) -> Result<GradStore> {
        ...
        for node in sorted_nodes.iter() {
            ...
            let grad = grads
                .remove(node)
                .expect("callosum internal error - grad not populated");
            ...
            let do_not_detach = CALLOSUM_GRAD_DO_NOT_DETACH.with(|b| *b);
